...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul 10 21:33:12 2019
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  2 21:21:14 2019
...
#matched_targets.bbox, proposals_per_image.bbox, gt_theta
    ...
    def encode(self, reference_boxes, proposals, gt_theta):
        ...
        p_ratio = gt_theta/ex_alpha
        p_diff = gt_theta-ex_alpha
...
#        target_dlamda = wl * (ex_lambda - gt_lambda)
        targets_dlamda_ratio = wl * p_ratio 
        
        targets_dlamda_diff = wl * (p_diff + 45)/135
        
#        targets_dlamda = wl * gt_lambda
#        targets_dlamda = wl * gt_lambda

        targets = torch.stack((targets_dx, targets_dy, targets_dw, targets_dh, targets_dlamda_ratio, targets_dlamda_diff), dim=1)
    ...
    def decode(self, rel_codes, boxes):
        ...
        dw = rel_codes[:, 2::6] / ww
        dh = rel_codes[:, 3::6] / wh
        dl_ratio = rel_codes[:, 4::6] / wl
        dl_diff = rel_codes[:, 5::6] / wl
        
        # Prevent sending too large values into torch.exp()
        dw = torch.clamp(dw, max=self.bbox_xform_clip)
        dh = torch.clamp(dh, max=self.bbox_xform_clip)

        dl_ratio = torch.clamp(dl_ratio, -2, 2)
        dl_diff = torch.clamp(dl_diff, -1, 1)
...
#        pred_alpha = pred_h/pred_w
        
        pred_alpha = torch.atan(pred_h/pred_w) * 180 / 3.141592653589793
        pred_theta_ratio = dl_ratio * pred_alpha
        pred_theta_diff = dl_diff * 135 - 45 + pred_alpha
        
        pred_theta = (pred_theta_ratio + pred_theta_diff) / 2
        
        
#        pred_theta = dl * 90 
#        pred_theta = dl * pred_alpha
